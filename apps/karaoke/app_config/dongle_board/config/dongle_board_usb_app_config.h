...
//以下功能只能选择一个，下面几项通过EP1实现，复用BULK通路
...
#if USB_MIC_SR_192000_EN && (USB_MIC_CHANNEL == USB_MIC_CHANNEL_STERO) && (USBD_MODE == USB_MODE_SPEAKER_MIC )
    //MIC 192采样率并且具有下行功能的时候不支持立体声，有很大的兼容性问题
    #error this configuration would make compatible problem!!!!
#endif
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
#define USB_MIC_NEED_RESAMPLE       (USB_MIC_SR_44100_EN | USB_MIC_SR_96000_EN | USB_MIC_SR_192000_EN)
...
#define MIC_INPUT_CHANNALS      USB_MIC_CHANNEL
