...
with open(map_file, 'r') as f:
    ...
    ram_config_text = re.findall('\nSRAM +\s+(0x\w+)\s+(0x\w+)\s+xrw\n',mem_config_text)
    ram_config_text += re.findall('\nSRAM7 +\s+(0x\w+)\s+(0x\w+)\s+xrw\n',mem_config_text)
    ...
    flash_config_text = re.findall('\nFLASH+\s+(0x\w+)\s+(0x\w+)\s+xr\n',mem_config_text)
    # flash_config_text += re.findall('\n.text+\s+(0x\w+)\s+(0x\w+)\s+xr\n', mem_config_text)
    ...
    for module in modules:
        ...
        map_lines.append((ram_size, '| %-40s | %-8d | %-8d |'%(re.sub('\.[ao]','',module)[:40], ram_size, flash_size) ))

print( '\n                            MEMORY MAP                            ')
print( '|================================================================|')
print( '| %-40s | %-8s | %-8s |'%('MODULE','RAM', 'FLASH'))
print( '|================================================================|')
ordered_list = sorted(map_lines, reverse=True)
for line in ordered_list:
    print( line[1])
print( '|================================================================|')
print( '| %-40s | %-8d | %-8d |'%('TOTAL (bytes)', total_ram, total_flash))
print( '| %-40s | %-8d | %-8d |'%('MEM LIMIT', limit_ram, limit_flash))
print( '| %-40s | %-8d | %-8d |'%('MEM LEFT', limit_ram-total_ram, limit_flash-total_flash))
print( '|================================================================|')
