...
int OAL_queue_send_to_front(OAL_QUEUE *qhandle, const void *msg, unsigned long wait);
...
int OAL_queue_peek(OAL_QUEUE *qhandle, void *msg, unsigned long wait);
...
/*
 * Reset a queue back to its original empty state.  OAL_SUCCESS is returned if the
